...
export default function CreateJobPage() {
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "Không thể tạo tin tuyển dụng.");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div className="max-w-5xl mx-auto animate-fade-in-up relative z-10 w-full pb-20">
      <div className="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          <h1 className="text-4xl md:text-5xl font-black text-on-surface mb-2">Tạo tin tuyển dụng</h1>
          <p className="text-on-surface-variant text-lg max-w-xl">Nhập thông tin rõ ràng để hệ thống AI so khớp ứng viên chính xác hơn.</p>
        </div>
