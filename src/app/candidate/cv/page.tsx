...
export default function CandidateCVPage() {
  ...
  const themedScrollbarClass =
    "[scrollbar-width:thin] [scrollbar-color:rgba(59,130,246,0.45)_transparent] [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-track]:rounded-full [&::-webkit-scrollbar-track]:bg-surface-container-high/40 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-primary/40 hover:[&::-webkit-scrollbar-thumb]:bg-primary/60";
  const extractSkillChips = useMemo(
    () =>
      extractForm.skillsText
        .split(",")
        .map((item) => item.trim())
        .filter(Boolean),
    [extractForm.skillsText],
  ...
  const handleApplyExtraction = async () => {
    ...
    if (Object.keys(payload).length === 0) {
      addToast("error", "No suitable data found for update.");
      return;
    }
    ...
    try {
      ...
      addToast("success", "Personal information updated from CV.");
      setShowExtractionModal(false);
    } catch (error) {
      if (error instanceof ApiError) {
        addToast("error", error.message);
      } else {
        addToast("error", "Could not update personal information.");
      }
    } finally {
  ...
  const handleFetchExtractedData = async (cvId: string, cvName: string) => {
    ...
    try {
      ...
      if (!parsed) {
        addToast("error", "No extracted data available for update.");
        return;
      }
      ...
      if (error instanceof ApiError) {
        ...
        addToast("error", "Could not fetch extracted data.");
      }
    } finally {
  ...
  const handleRetryExtraction = async (cvId: string, cvName: string) => {
    ...
    try {
      ...
      addToast("success", `Extraction retry requested for CV ${cvName}.`);
    } catch (error) {
      if (error instanceof ApiError) {
        addToast("error", error.message);
      } else {
        addToast("error", "Could not retry extraction.");
      }
    } finally {
      ...
      {showExtractionModal && extractionCvId && (
        <div className="fixed inset-0 z-50 bg-black/45 backdrop-blur-[2px] flex items-center justify-center p-4">
          <div className="w-full max-w-lg rounded-3xl border border-outline/30 bg-surface p-6 shadow-2xl space-y-4 max-h-[85vh] flex flex-col">
            ...
            <div className={`space-y-4 overflow-y-auto pr-1 ${themedScrollbarClass}`}>
                        ...
                        <div>
                          ...
                          <div className={`mt-2 flex flex-wrap gap-2 max-h-28 overflow-y-auto pr-1 ${themedScrollbarClass}`}>
                            {extractionData.skills.map((skill, index) => (
                              <span
                                key={`${skill}-${index}`}
                                className="inline-flex items-center gap-2 bg-surface-container-highest px-3 py-1 rounded-full text-xs font-semibold text-on-surface-variant break-words"
                              >
                                {skill}
                              </span>
                            ))}
      ...
      {showExtractForm && (
        <div className="fixed inset-0 z-50 bg-black/45 backdrop-blur-[2px] flex items-center justify-center p-4">
          <div className="w-full max-w-3xl rounded-3xl border border-outline/30 bg-surface p-7 shadow-2xl max-h-[88vh] flex flex-col">
            ...
            <div className="flex flex-wrap justify-end gap-3 pt-2 border-t border-outline/20 mt-2">
              <button
                type="button"
